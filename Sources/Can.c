...
static uint32 CAN_BASE_ADDRESSES[NUM_OF_CONTROLLERS] = {CAN_0_BASE_ADDRESS, CAN_1_BASE_ADDRESS, CAN_2_BASE_ADDRESS};
...
static uint32 PCC_CAN_OFFSETS[NUM_OF_CONTROLLERS] = {0x90,0x94,0xAC};
...
static uint8 MAXMB[NUM_OF_CONTROLLERS] = {128, 64, 64};
				...
				if (STANDARD == Config->CanHardwareObject_ptr[(BufferCounter) ].CanIdType)
				{
					...
					*(RAMn) |= (1 << 21);
					...
					if (TRANSMIT == Config->CanHardwareObject_ptr[BufferCounter].CanObjectType)
					{
						...
						Can_HTH_Array[TxBufferCounter++] = TempConfig->CanHardwareObject_ptr[BufferCounter];
					...
					else 
					{
						...
						Can_HRH_Array[RxBufferCounter++] = TempConfig->CanHardwareObject_ptr[BufferCounter];
...
Std_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo )
{
	...
	if (MB_CS_INACTIVE == ( HW_Register( BufferAddress ) & (0x0f000000) ) )
	{
		...
		IFLAG1_Reg |= (1 << DistanceFromBeggining);
